...
- `dotfiles/claude/skills/` -> `~/.claude/skills/` (whole directory) — `fastapi` (first-party), `dotfiles-sync` (meta), `ticket-breakdown-and-delegation` (first-party, extends `planning-and-task-breakdown` with per-assignee sizing), seven vendored SDLC-workflow skills (`spec-driven-development`, `planning-and-task-breakdown`, `test-driven-development`, `code-review-and-quality`, `debugging-and-error-recovery`, `git-workflow-and-versioning`, `incremental-implementation`) under `ADDYOSMANI_AGENT_SKILLS_LICENSE` (MIT), `research`/`handoff` under `MATTPOCOCK_SKILLS_LICENSE` (MIT), and `caveman` (terse response mode) under `CAVEMAN_LICENSE` (MIT — only the plain `skills/` directory of that repo; its compression-engine binaries are BSL-1.1 and not used here)
- `dotfiles/claude/commands/` -> `~/.claude/commands/` (whole directory) — `/spec`, `/plan`, `/test`, `/review`: short aliases into the SDLC skills above; `/build` orchestrates `executor` dispatch per task, then fans out `code-reviewer`/`security-reviewer`/`infra-reviewer`/`distributed-systems-reviewer` (each conditional except code/security) and merges a go/no-go verdict
...
- `dotfiles/claude/hooks/` -> `~/.claude/hooks/` (whole directory) — scripts referenced by `settings.json`'s `hooks` key: destructive-bash blocking (`rm -rf`, block-device writes, recursive chmod/chown, and the git equivalents — `reset --hard`, `clean -f`, `branch -D`, `checkout .`/`restore .`) and a force-push-to-main/master warning
