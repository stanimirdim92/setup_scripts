...
- `dotfiles/claude/agents/` -> `~/.claude/agents/` (whole directory) — custom subagents: `infra-reviewer`, `security-reviewer`, `executor` (first-party — dispatched by `/build` to implement one task end-to-end; never invokes another agent), plus vendored `code-reviewer` (five-axis review, MIT-licensed — see `dotfiles/claude/skills/ADDYOSMANI_AGENT_SKILLS_LICENSE`)
...
- `dotfiles/claude/commands/` -> `~/.claude/commands/` (whole directory) — `/spec`, `/plan`, `/test`, `/review`: short aliases into the SDLC skills above; `/build` orchestrates `executor` dispatch per task, then fans out `code-reviewer`/`security-reviewer`/`infra-reviewer` and merges a go/no-go verdict
