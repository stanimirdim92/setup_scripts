...
- `dotfiles/claude/commands/` -> `~/.claude/commands/` (whole directory) — `/spec`, `/plan`, `/test`, `/review`: short aliases into the SDLC skills above; `/build` orchestrates `executor` dispatch per task (asking first before 3+ concurrent Opus-tier executors), then fans out `code-reviewer`/`security-reviewer`/`infra-reviewer`/`distributed-systems-reviewer` (only `code-reviewer` is unconditional; the other three match `review.md`'s own triggers, and 3+ reviewers firing at once also asks first) and merges a go/no-go verdict
