...
- `dotfiles/claude/skills/` -> `~/.claude/skills/` (whole directory) — on-demand, per-technology skills (`php`, `mysql`, `nginx`, `redis`, `postgresql`, `python`, `fastapi`), `dotfiles-sync` (meta), two vendored reference skills (`qdrant-multitenancy`, `postgres-database-migration`), and six vendored SDLC-workflow skills (`spec-driven-development`, `planning-and-task-breakdown`, `test-driven-development`, `code-review-and-quality`, `debugging-and-error-recovery`, `git-workflow-and-versioning`) — see `dotfiles/claude/skills/VENDORED.md` for source/license/refresh on all of the above
