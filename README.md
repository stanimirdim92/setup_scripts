...
- `dotfiles/claude/agents/` -> `~/.claude/agents/` (whole directory) — custom subagents: `infra-reviewer`, `security-reviewer` (first-party), plus vendored `code-reviewer` (five-axis review — see `dotfiles/claude/skills/VENDORED.md`)
- `dotfiles/claude/skills/` -> `~/.claude/skills/` (whole directory) — on-demand, per-technology skills (`php`, `mysql`, `nginx`, `redis`, `postgresql`, `python`, `fastapi`), `dotfiles-sync` (meta), two vendored reference skills (`qdrant-multitenancy`, `postgres-database-migration`), and seven vendored SDLC-workflow skills (`spec-driven-development`, `planning-and-task-breakdown`, `test-driven-development`, `code-review-and-quality`, `debugging-and-error-recovery`, `git-workflow-and-versioning`, `ci-cd-and-automation`) — see `dotfiles/claude/skills/VENDORED.md` for source/license/refresh on all of the above
- `dotfiles/claude/commands/` -> `~/.claude/commands/` (whole directory) — `/spec`, `/plan`, `/test`, `/review`: short aliases into the SDLC skills above, adapted from the same upstream (stripped of its plugin-namespace prefix and its cross-references to skills not vendored here)
