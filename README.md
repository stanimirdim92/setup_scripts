...
- `dotfiles/claude/commands/` -> `~/.claude/commands/` (whole directory) — `/spec`, `/plan`, `/test`, `/review`: short aliases into the SDLC skills above; `/build` groups tasks into workstreams and dispatches one `executor` per workstream (resumed across that workstream's later tasks instead of a fresh spawn per task), capped at 2 concurrent workstreams and 2 concurrent reviewers regardless of how many are eligible, then fans out `code-reviewer`/`security-reviewer`/`infra-reviewer`/`distributed-systems-reviewer` (only `code-reviewer` is unconditional; the other three match `review.md`'s own triggers) with a goal+acceptance-criteria+diff each, not the full plan, and merges a go/no-go verdict
- `dotfiles/claude/references/` -> `~/.claude/references/` (whole directory) — shared checklists (`definition-of-done.md`, `security-checklist.md`, `performance-checklist.md`) that the SDLC skills point at with a `../../references/` path, plus `documentation-practices.md` (the Ideas/Decisions/Memory practice moved out of `CLAUDE.md` so it isn't force-loaded into every subagent); must stay a sibling of `skills/`, not nested inside it
