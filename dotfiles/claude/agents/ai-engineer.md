...
- **Invoke directly when:** the user wants to build or extend a production LLM
  application — RAG pipeline, agent architecture, or AI-powered feature.
- **Invoke via:** no slash command currently wraps this agent; `/ai-assistant`
  and `/langchain-agent` are standalone vendored commands with their own
  inline instructions and don't dispatch it. Reach for those directly for
