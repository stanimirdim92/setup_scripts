...
- **Invoke directly when:** the user needs vector search, embedding
  strategy, or semantic retrieval work — index selection/tuning, hybrid
  search, or RAG retrieval-layer design specifically.
- **Invoke via:** no slash command currently wraps this agent; its paired
  skills (`embedding-strategies`, `similarity-search-patterns`,
  `vector-index-tuning`, `hybrid-search-implementation`, `rag-implementation`)
