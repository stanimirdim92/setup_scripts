...
- **Invoke directly when:** the user wants a prompt designed, reviewed, or
  optimized — system prompts, few-shot templates, or model-specific
  prompting technique selection.
- **Invoke via:** `/prompt-optimize` covers the narrower "optimize this
  existing prompt" workflow with its own inline instructions and doesn't
